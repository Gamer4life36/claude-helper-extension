...
chrome.runtime.onInstalled.addListener(() => { seedPolicy(); try {
...
function forbiddenReason(host, policy) {
    if (suffixMatch(host, policy.forbiddenDomains))
        return `${host} is on the forbidden domains list`;
    const k = (policy.forbiddenKeywords || []).find((w) => host.includes(w));
    if (k)
        return `${host} matches forbidden keyword "${k}"`;
    return null;
}
...
function searchQueryForbidden(url, policy) {
    try {
        ...
        if (isSearch) {
            ...
            if (k)
                return `search query matches forbidden keyword "${k}"`;
        }
    }
    catch { }
    return null;
...
async function execTool(tool, args = {}) {
    ...
    if (!policy.capabilities[tool])
        return { ok: false, error: `blocked: capability "${tool}" is disabled in policy` };
    if (tool === "open_tab" || tool === "navigate") {
        const why = forbiddenReason(hostOf(args.url), policy) || searchQueryForbidden(args.url, policy);
        if (why)
            return { ok: false, forbidden: true, error: "blocked: " + why };
    }
    switch (tool) {
        case "open_tab": {
...
async function activeTabId() { const [t] = await chrome.tabs.query({ active: true, currentWindow: true }); if (!t)
...
connectBridge();
setInterval(() => { if (bridge && bridge.readyState === 1)
    bridge.send(JSON.stringify({ ping: 1 })); }, 20000);
export {};
