...
function resolve(raw) {
    ...
    if (hit && !s.includes(" ") && !looksLikeUrl(s))
        return "https://" + hit[1];
    if (looksLikeUrl(s))
        return toUrl(s);
...
input.addEventListener("keydown", (e) => {
    ...
    if (e.key === "ArrowDown") {
        ...
        if (active >= 0 && items[active])
            openUrl(items[active].dataset.url);
        else
            openUrl(resolve(input.value));
    }
    else if (e.key === "Escape")
        box.style.display = "none";
});
$("open").onclick = () => openUrl(resolve(input.value));
document.addEventListener("click", (e) => { if (!$("searchWrap").contains(e.target))
    box.style.display = "none"; });
// ── existing controls ──────────────────────────────────────────────────────
