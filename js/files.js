...
async function walk(dir, prefix = "", out = [], depth = 0) {
    if (out.length >= 5000 || depth > 10)
        return out;
    for await (const [name, h] of dir.entries()) {
        const path = prefix ? prefix + "/" + name : name;
        if (h.kind === "file")
            out.push({ path, handle: h });
        else if (!name.startsWith(".") && name !== "node_modules" && name !== ".git")
            await walk(h, path, out, depth + 1);
        if (out.length >= 5000)
            break;
    }
    return out;
...
function looksBinary(buf) { const b = new Uint8Array(buf), n = Math.min(b.length, 8192); for (let i = 0; i < n; i++)
    if (b[i] === 0)
...
document.addEventListener("keydown", (e) => { if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === "s") {
    e.preventDefault();
    save();
} });
