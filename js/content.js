...
(() => {
        ...
        try {
            if (el.id) {
                ...
                if (l && l.innerText.trim())
                    return l.innerText.trim().slice(0, 60);
            }
    ...
    async function perform(action) {
        ...
        if (isForbidden())
            return { ok: false, forbidden: true, error: "forbidden by policy: this site is blocked (" + host() + ")" };
        if (kind === "read")
        ...
        if (kind === "scroll") {
            if (action.to === "bottom")
                ...
                window.scrollBy({ top: (action.amount || 700) * (action.direction === "up" ? -1 : 1), behavior: "smooth" });
        ...
        if (kind === "find_text") {
            ...
            if (!el)
                return { ok: false, error: `"${action.text}" not found on this page` };
            el.scrollIntoView({ block: "center", behavior: "smooth" });
            const old = el.style.backgroundColor;
            el.style.backgroundColor = "#ffe066";
            setTimeout(() => { el.style.backgroundColor = old; }, 2500);
        ...
        if (kind === "links") {
            ...
            for (const a of document.querySelectorAll("a[href]")) {
                const href = a.href;
                if (!/^https?:/.test(href) || seen.has(href))
                    continue;
                seen.add(href);
        ...
        if (!el && kind === "submit")
            el = document.querySelector("form button[type=submit], form [type=submit], form"); // submit: fall back to the page's form
        if (!el)
            return { ok: false, error: "element not found — run read_page first to get refs." };
        ...
        if (kind === "type" && policy?.rules?.forbidSensitiveData) {
            ...
            if (hit)
                return { ok: false, forbidden: true, error: `HARD BLOCK: I won't enter sensitive data (field looks like "${hit}"). Please type this yourself.` };
        }
        if ((kind === "click" || kind === "submit") && policy?.rules?.forbidPurchases) {
            const hit = (policy.purchaseKeywords || []).find((w) => btnLabel.includes(w));
            if (hit)
                return { ok: false, forbidden: true, error: `HARD BLOCK: I won't complete a purchase ("${hit}"). Do the final buy/pay step yourself.` };
        }
        ...
        if (s.mode === "ask") {
            ...
            if (!ok)
                return { ok: false, denied: true, error: "Denied by user." };
        }
