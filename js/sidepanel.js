...
let lastEls = [];
async function ensureRead() { const r = await exec("read_page", {}); if (r.ok)
    lastEls = r.page.elements; return lastEls; }
function scoreEl(x, s, keys) { let sc = 0; for (const key of keys) {
...
function normKey(k) { k = k.toLowerCase().trim(); for (const [c, syns] of Object.entries(FIELD_SYNS)) {
    if (c === k || syns.includes(k))
        return c;
} return k; }
...
async function autofillProfile(prof) {
    ...
    for (const [key, value] of Object.entries(prof)) {
        ...
        if (!filled)
            done.push(`• ${key}: no field`);
    }
    return done;
}
const clickByText = async (label) => { await ensureRead(); const tgt = findEl(label); if (!tgt)
    return { ok: false, error: `no “${label}” control found — try "read" first` }; return exec("click", tgt); };
// intent → URL
const enc = encodeURIComponent;
function intentUrl(t) {
    let m;
    if ((m = t.match(/^(?:images?|pictures?|pics?)\s+of\s+(.+)/i)))
        return "https://www.google.com/search?tbm=isch&q=" + enc(m[1]);
    if ((m = t.match(/^(?:videos?|watch|play)\s+(?:of\s+)?(.+)/i)))
        return SEARCH_URLS.youtube + enc(m[1]);
    if ((m = t.match(/^directions?\s+from\s+(.+?)\s+to\s+(.+)$/i)))
        return "https://www.google.com/maps/dir/" + enc(m[1]) + "/" + enc(m[2]);
    if ((m = t.match(/^(?:maps?|directions?(?:\s+to)?|navigate to|where is)\s+(.+)/i)))
        return "https://www.google.com/maps/search/" + enc(m[1]);
    if ((m = t.match(/^(?:wiki|wikipedia)\s+(.+)/i)))
        return SEARCH_URLS.wikipedia + enc(m[1]);
    if ((m = t.match(/^(?:define|definition of|meaning of)\s+(.+)/i)))
        return "https://www.google.com/search?q=" + enc("define " + m[1]);
    if ((m = t.match(/^translate\s+(.+)/i)))
        return "https://translate.google.com/?sl=auto&tl=en&text=" + enc(m[1]);
    if ((m = t.match(/^weather(?:\s+(?:in|for))?\s+(.+)/i)))
        return "https://www.google.com/search?q=" + enc("weather " + m[1]);
    if ((m = t.match(/^(?:buy|shop(?:\s+for)?|order|purchase)\s+(.+)/i)))
        return SEARCH_URLS.amazon + enc(m[1]);
    if ((m = t.match(/^news(?:\s+(?:about|on))?\s+(.+)/i)))
        return "https://news.google.com/search?q=" + enc(m[1]);
    if ((m = t.match(/^(?:tweet|post to (?:twitter|x))\s+(.+)/i)))
        return "https://twitter.com/intent/tweet?text=" + enc(m[1]);
    if ((m = t.match(/^(?:add (?:a )?(?:calendar )?event|schedule|new event|remind me to)\s+(.+)/i)))
        return "https://calendar.google.com/calendar/render?action=TEMPLATE&text=" + enc(m[1]);
    if ((m = t.match(/^(?:search (?:my )?(?:e-?mail|gmail|inbox)|find (?:e-?mails?|mail))\s+(?:for\s+|from\s+)?(.+)/i)))
        return "https://mail.google.com/mail/u/0/#search/" + enc(m[1]);
    if ((m = t.match(/^(?:stock|ticker|share price)\s+(?:of\s+|for\s+)?(.+)/i)))
        return "https://www.google.com/search?q=" + enc(m[1] + " stock");
    if ((m = t.match(/^flights?\s+(?:from\s+)?(.+?)\s+to\s+(.+)$/i)))
        return "https://www.google.com/travel/flights?q=" + enc("flights from " + m[1] + " to " + m[2]);
    if ((m = t.match(/^(?:showtimes?|movie times?)\s+(?:for\s+)?(.+)/i)))
        return "https://www.google.com/search?q=" + enc(m[1] + " showtimes");
    if ((m = t.match(/^recipe(?:s)?\s+(?:for\s+)?(.+)/i)))
        return "https://www.google.com/search?q=" + enc(m[1] + " recipe");
    return null;
}
...
function summarize(text, n = 6) {
    ...
    for (const w of clean.toLowerCase().match(/[a-z']{3,}/g) || [])
        if (!stop.has(w))
            freq[w] = (freq[w] || 0) + 1;
    const scored = sentences.map((s, i) => { const words = s.toLowerCase().match(/[a-z']{3,}/g) || []; let sc = words.reduce((a, w) => a + (freq[w] || 0), 0) / (words.length || 1); if (i < 3)
        sc *= 1.15; return { s: s.trim(), sc, i }; });
...
chrome.storage.local.get("skillsSeeded").then(async ({ skillsSeeded }) => {
    ...
    if (!s.gamepost)
        s.gamepost = { description: "announce or promote a Steam game on social media", steps: 'open steam and search $input then ask Write a short, punchy Facebook post announcing my Steam game "$input". Include [STORE LINK] and [RELEASE DATE] placeholders, a one-line hook, 2-3 relevant hashtags, and keep it under 80 words.' };
    await setSkills(s);
...
async function runSkill(name, input, depth = 0) {
    ...
    if (!sk)
        return { text: `No skill “${name}”.` };
    if (depth > 4)
        return { text: "Skill nesting too deep — stopped.", blocked: true };
    ...
    for (const step of steps) {
        ...
        if (r.nano) {
            ...
            continue;
        } // AI step inside a skill → on-device model
        addMsg("ai", r.text, r.els ? "els" : r.blocked ? "blocked" : "");
        if (r.blocked)
            return { text: `Skill “${name}” stopped — a step was blocked.`, blocked: true };
    }
...
async function nanoSummarize(text, onProg) {
    if (typeof Summarizer !== "undefined") {
        try {
            ...
            if (a !== "unavailable") {
                const sm = await Summarizer.create({ type: "key-points", format: "markdown", length: "medium", monitor(mn) { mn.addEventListener("downloadprogress", (e) => onProg && onProg(e.loaded)); } });
                const out = await sm.summarize(text.slice(0, 12000));
                try {
                    sm.destroy();
                }
                catch { }
...
async function interpret(text, depth = 0, opts = {}) {
    ...
    if ((m = t.match(/^skill\s+(?:delete|remove|forget)\s+([\w-]+)$/i))) {
        ...
        if (!s[k])
            return { text: `No skill “${m[1]}”.` };
        delete s[k];
        await setSkills(s);
        return { text: `🗑 Deleted skill “${m[1]}”.` };
    }
    if ((m = t.match(/^(?:use|@|\/)\s*([\w-]+)(?:\s+(.+))?$/i)) && (await getSkills())[m[1].toLowerCase()])
        return runSkill(m[1].toLowerCase(), (m[2] || "").trim(), depth);
    if ((m = t.match(/^do\s+(.+)/i))) {
        const skills = await getSkills();
        if (!Object.keys(skills).length)
            return { text: 'No skills yet. Create one with "skill new …".' };
        if (!(await nanoReady()))
            return { text: `🧠 On-device AI isn’t ready, so I can’t auto-pick a skill. Invoke one directly:  use <name> <input>.\nYour skills: ${Object.keys(skills).join(", ")}` };
        const pick = await nanoPickSkill(m[1], skills);
        if (!pick)
            return { text: `No skill matched “${m[1]}”. Your skills: ${Object.keys(skills).join(", ")}` };
    ...
    if ((m = t.match(/^(?:run|play|macro)\s+([\w-]+)$/i))) {
        ...
        for (const step of steps) {
            ...
            addMsg("ai", r.text, r.els ? "els" : r.blocked ? "blocked" : "");
            if (r.blocked)
                return { text: `Macro “${m[1]}” stopped — a step was blocked.`, blocked: true };
        }
    ...
    if (/^(go )?back$/.test(l)) {
        const r = await exec("back", {});
        return { text: r.ok ? "Went back." : "🚫 " + r.error };
    }
    if (/^(go )?forward$/.test(l)) {
        const r = await exec("forward", {});
        return { text: r.ok ? "Went forward." : "🚫 " + r.error };
    }
    ...
    if (/^(press|hit)\s+enter$/.test(l)) {
        const r = await exec("submit", {});
        return { text: r.ok ? "Submitted." : "🚫 " + r.error, blocked: !r.ok };
    }
    ...
    if ((m = t.match(/^(?:find|locate|highlight|jump to)\s+(.+?)\s+on\s+(?:this|the)\s+page$/i)) || (m = t.match(/^find on page\s+(.+)/i)) || (m = t.match(/^(?:find|highlight)\s+"([^"]+)"$/i))) {
        const r = await exec("find_text", { text: m[1].replace(/^["'“]|["'”]$/g, "").trim() });
        return { text: r.ok ? `Found: “${r.text}”` : "🚫 " + r.error, blocked: !r.ok };
    }
    ...
    if (/^(add to cart|add to bag|add to basket|add this to (?:my )?cart)$/i.test(l)) {
        const r = await clickByText("add to cart");
        return { text: r.ok ? "Clicked Add to cart." : "🚫 " + r.error, blocked: !r.ok };
    }
    if (/^(buy now|buy it now)$/i.test(l)) {
        const r = await clickByText("buy now");
        return { text: r.ok ? "Clicked Buy now (confirm if prompted)." : "🚫 " + r.error, blocked: !r.ok };
    }
    if (/^(checkout|check out|proceed to checkout|place order)$/i.test(l)) {
        const r = await clickByText("checkout");
        return { text: r.ok ? "Clicked Checkout (confirm if prompted)." : "🚫 " + r.error, blocked: !r.ok };
    }
    if (/^(sign in|log ?in)$/i.test(l)) {
        const r = await clickByText("sign in");
        return { text: r.ok ? "Clicked Sign in." : "🚫 " + r.error, blocked: !r.ok };
    }
    ...
    if (/^translate (this|the) page$|^translate page$/i.test(l)) {
        ...
        const u = tab?.url || "";
        if (!/^https?:/.test(u))
            return { text: "Open a normal web page first, then say “translate this page”." };
    ...
    if (/^(summari[sz]e|summary|tl;?dr|key points|main points)( (this|the) (page|article)| it)?$/i.test(l)) {
        const r = await exec("extract", {});
        if (!r.ok)
            return { text: "🚫 " + r.error, blocked: true };
        if ((r.text || "").length < 40)
            return { text: "Couldn't find readable article text on this page." };
        if (await nanoReady()) {
            try {
                const out = await nanoSummarize(r.text);
                if (out)
                    return { text: `🧠 ${r.title}\n\n${out}` };
            }
            catch { }
        }
        const pts = summarize(r.text, 6);
        if (!pts.length)
            return { text: "Couldn't find readable article text on this page." };
    ...
    if (/^(reader|reader view|read this|clean view|declutter|simplify)( (this|the) page)?$/i.test(l)) {
        ...
        if (!r.ok)
            return { text: "🚫 " + r.error, blocked: true };
        if ((r.text || "").length < 40)
            return { text: "Not enough article text here for a reader view." };
        await exec("open_tab", { url: readerPage(r.title, r.text) });
    ...
    if (/^(read (this )?(aloud|to me)|speak|say this|start reading)$/i.test(l)) {
        const r = await exec("speak", {});
        return { text: r.ok ? "🔊 Reading the page aloud… say “stop reading” to stop." : "🚫 " + r.error, blocked: !r.ok };
    }
    ...
    if (/^(list |show |get )?links$/i.test(l)) {
        const r = await exec("links", {});
        if (!r.ok)
            return { text: "🚫 " + r.error, blocked: true };
        const list = r.links.slice(0, 60).map((x) => `• ${x.text || x.href}\n  ${x.href}`).join("\n");
    ...
    if (/^(extract |find |get |list )?e-?mails?$/i.test(l)) {
        ...
        if (!r.ok)
            return { text: "🚫 " + r.error, blocked: true };
        const em = [...new Set(r.text.match(/[\w.+-]+@[\w-]+\.[\w.-]+/g) || [])];
    ...
    if (/^(extract |find |get |list )?prices?$/i.test(l)) {
        ...
        if (!r.ok)
            return { text: "🚫 " + r.error, blocked: true };
        const pr = [...new Set(r.text.match(/[$£€]\s?\d[\d,]*(?:\.\d{2})?/g) || [])];
    ...
    if (/^(dark|night)( mode)?$|^toggle dark( mode)?$/i.test(l)) {
        const r = await exec("darkmode", {});
        return { text: r.ok ? "🌙 " + r.did + " (run again to toggle)." : "🚫 " + r.error };
    }
    if (/^zoom in$/i.test(l)) {
        const r = await exec("zoom", { dir: "in" });
        return { text: r.ok ? "🔍 " + r.did : "🚫 " + r.error };
    }
    if (/^zoom out$/i.test(l)) {
        const r = await exec("zoom", { dir: "out" });
        return { text: r.ok ? "🔍 " + r.did : "🚫 " + r.error };
    }
    if (/^(reset zoom|zoom reset|actual size)$/i.test(l)) {
        const r = await exec("zoom", { dir: "reset" });
        return { text: r.ok ? "🔍 " + r.did : "🚫 " + r.error };
    }
    if (/^copy (this )?(url|link|address)$/i.test(l)) {
        const r = await exec("copy", {});
        return { text: r.ok ? "📋 Copied the page URL." : "🚫 " + r.error };
    }
    if (/^copy (the )?(page )?text$/i.test(l)) {
        const r = await exec("extract", {});
        if (!r.ok)
            return { text: "🚫 " + r.error };
        const c = await exec("copy", { text: r.text.slice(0, 100000) });
        return { text: c.ok ? "📋 Copied the page text." : "🚫 " + c.error };
    }
    ...
    if ((m = t.match(/^(?:log ?in(?:to| to)?|sign ?in(?:to| to)?)\s+(.+)$/i))) {
        ...
        return { text: r.ok ? `Opened ${m[1]} — click Sign in there (I never enter passwords).` : "🚫 " + r.error, blocked: !r.ok };
    }
    ...
    if ((m = t.match(/^(?:share|post)\s+(?:(https?:\/\/\S+)\s+)?(?:this(?:\s+page)?\s+)?(?:to|on)\s+(facebook|fb|linkedin|reddit|twitter|x)(?:\s+(https?:\/\/\S+))?$/i))) {
        ...
        if (!/^https?:/.test(url))
            return { text: `Give a link, e.g. “share to ${net} https://store.steampowered.com/app/…”.` };
    ...
    if ((m = t.match(/^(?:set|save|remember|update)\s+my\s+(?:info|profile|details|contact(?:\s+info)?)?\s*(?:to|:|=|with|as)?\s*(.+)$/i)) && /[:=]/.test(m[1])) {
        ...
        if (!pairs.length)
            return { text: 'Say e.g. "set my info name=Mike, email=me@x.com, phone=555-1234".' };
        const prof = (await chrome.storage.local.get("profile")).profile || {};
    ...
    if (/^(?:fill|autofill|complete)\s+(?:this\s+)?(?:form\s+)?(?:with\s+)?my\s+(?:info|profile|details|contact(?:\s+info)?)$|^autofill$/i.test(l)) {
        ...
        if (!Object.keys(prof).length)
            return { text: 'No saved profile yet. First: "set my info name=Mike, email=me@x.com, phone=...".' };
    ...
    if ((m = t.match(/^(?:open\s+|go to\s+|on\s+)?([a-z0-9.-]+)\s+(?:and\s+)?search(?:\s+for)?\s+(.+)$/i)) && known(m[1])) {
        const r = await exec("open_tab", { url: siteSearchUrl(m[1], m[2]) });
        return { text: r.ok ? `Searching ${m[1]} for “${m[2]}”` : "🚫 " + r.error, blocked: !r.ok };
    }
    ...
    if (iu) {
        const r = await exec("open_tab", { url: iu });
        return { text: r.ok ? "Opened." : "🚫 " + r.error, blocked: !r.ok };
    }
    // open several at once: "open youtube, reddit, github"
    if ((m = l.match(/^open\s+(.+,.+)$/))) {
        const sites = m[1].split(/\s*,\s*/).map((s) => s.trim()).filter(Boolean);
        for (const s of sites)
            await exec("open_tab", { url: resolve(s) });
    ...
    if ((m = l.match(/^(?:open|go to|goto|visit|launch)\s+(.+)/))) {
        ...
        const r = await exec("open_tab", { url });
        return { text: r.ok ? "Opened " + url : "🚫 " + r.error, blocked: !r.ok };
    }
    // generic search
    if ((m = l.match(/^(?:search|google|find)\s+(.+)/))) {
        const r = await exec("open_tab", { url: "https://www.google.com/search?q=" + encodeURIComponent(m[1]) });
        return { text: r.ok ? "Searched: " + m[1] : "🚫 " + r.error, blocked: !r.ok };
    }
    // read
    ...
    if ((m = t.match(/^fill\s+(?:(?:in|out)\s+)?(?:the\s+)?(?:form\s+)?(?:with\s+)?(.+)$/i)) && /[:=]/.test(m[1])) {
        const pairs = parsePairs(m[1]);
        if (!pairs.length)
            return { text: `Say e.g. "fill name=John, email=john@x.com, message: hello".` };
        await ensureRead();
        const done = [];
        for (const { k, v } of pairs) {
            ...
            const r = await exec("type", { ...tgt, text: v });
            done.push(`• ${k} → ${r.ok ? "filled" : r.error}`);
        }
    ...
    if ((m = t.match(/^type\s+(.+?)\s+(?:into|in)\s+(.+)$/i))) {
        ...
        if (!tgt)
            return { text: `Read the page first, then e.g. "type ${m[1]} into email".` };
        const r = await exec("type", { ...tgt, text: m[1] });
        return { text: r.ok ? `Typed into ${m[2]}` : "🚫 " + r.error, blocked: !r.ok };
    }
    ...
    if ((m = l.match(/^click(?:\s+on)?\s+(.+)/))) {
        ...
        const tgt = findEl(q);
        if (!tgt)
            return { text: `Couldn't find "${q}". Say "read" first, then click by number or visible text.` };
        const r = await exec("click", tgt);
        return { text: r.ok ? `Clicked ${q}` : "🚫 " + r.error, blocked: !r.ok };
    }
    // submit
    if (/^submit\b/.test(l)) {
        const r = await exec("submit", {});
    ...
    if (!opts.noFallback && (/\?$/.test(t) || /^(what|who|when|where|why|how|is|are|can|does|do|should|which|will|whats|what's)\b/.test(l))) {
        const r = await exec("open_tab", { url: "https://www.google.com/search?q=" + enc(t) });
        return { text: r.ok ? "Searched Google: " + t : "🚫 " + r.error, blocked: !r.ok };
    }
...
chrome.storage.local.get("mode").then((v) => { if (["free", "nano", "api"].includes(v.mode))
    mode = v.mode; updateMode(); refreshNano(); });
...
function updateMode() {
    const set = (id, on) => { const b = document.getElementById(id); if (b)
    ...
    if (nh)
        nh.style.display = (mode === "nano" && !ready) ? "block" : "none";
    if (mode === "free")
        modeEl.innerHTML = '<span class="dot b"></span>Free mode — pattern commands, no AI';
    else if (mode === "nano")
        modeEl.innerHTML = ready ? '<span class="dot g"></span>On-device AI ready — Gemini Nano, local & free'
        ...
        modeEl.innerHTML = bridgeUp ? '<span class="dot g"></span>Claude API — bridge connected' : '<span class="dot" style="background:#b3261e"></span>Claude API — start the bridge or add a key';
}
async function checkMode() { try {
...
async function runNano(prompt) {
    ...
    if (!nanoPresent()) {
        div.textContent = "🧠 On-device AI (Gemini Nano) needs Chrome 138+ on capable hardware. Use 🔵 Free or 🟢 Claude API instead.";
        div.className += " blocked";
        return;
    }
    try {
        const ans = await nanoAsk(prompt, (frac) => { div.textContent = `🧠 downloading model… ${Math.round(frac * 100)}% (one-time)`; logEl.scrollTop = logEl.scrollHeight; });
        div.textContent = ans;
        logEl.scrollTop = logEl.scrollHeight;
    }
    catch (e) {
        div.textContent = "🧠 on-device AI error: " + (e?.message || e);
        div.className += " blocked";
    }
}
...
document.getElementById("f").addEventListener("submit", async (e) => {
    ...
    if (mode === "api") {
        if (!bridgeUp)
            return void addMsg("sys", "🟢 Claude API mode is on but the bridge isn’t running. Start it (node server.js with your API key), or switch modes up top.");
    ...
    for (const step of steps) {
        ...
        if (r.nano) {
            await runNano(r.nano);
            continue;
        } // explicit "ask …" — any mode
        if (r.fallthrough) {
            await runNano(step);
            continue;
        } // 🧠 mode: plain text → local AI
        addMsg("ai", r.text, r.els ? "els" : r.blocked ? "blocked" : "");
        if (r.blocked)
            break;
    }
