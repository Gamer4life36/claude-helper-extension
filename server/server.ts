...
function callExtension(tool, args) {
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => { if (pending.has(id)) { pending.delete(id); reject(new Error("extension timed out (a confirmation may be waiting, or the tab has no content script)")); } }, 180000);
  });
}
...
async function runTask(task) {
  ...
  try {
    for (let step = 0; step < MAX_STEPS; step++) {
      ...
      if (!toolUses.length) { log("✓ Done."); break; }
      const results = [];
      for (const tu of toolUses) {
        log("→ " + tu.name + " " + JSON.stringify(tu.input));
        let result;
        try { result = await callExtension(tu.name, tu.input); }
...
const server = http.createServer((req, res) => {
  ...
  if (req.url === "/task" && req.method === "POST") {
    let body = ""; req.on("data", (c) => (body += c)); req.on("end", () => {
      try { const { task } = JSON.parse(body || "{}"); if (task) runTask(task); res.writeHead(200); res.end("ok"); }
...
wss.on("connection", (ws) => {
  extSocket = ws; log("● extension connected");
  ws.on("message", (m) => {
    let d; try { d = JSON.parse(m); } catch { return; }
    if (d.ping || d.hello) return;
    const p = pending.get(d.id);
    if (p) { pending.delete(d.id); d.error ? p.reject(new Error(d.error)) : p.resolve(d.result); }
  });
  ws.on("close", () => { if (extSocket === ws) extSocket = null; log("○ extension disconnected"); });
});
