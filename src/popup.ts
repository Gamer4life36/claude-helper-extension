...
function resolve(raw) {
  ...
  if (!s) return null;
  const hit =
    SITES.find(([n, d]) => n.toLowerCase() === s.toLowerCase() || d === s.toLowerCase()) || SITES.find(([n, d]) => n.toLowerCase().startsWith(s.toLowerCase()) || d.startsWith(s.toLowerCase()));
...
function renderSuggest(q) {
  ...
  if (s && !matches.some(([n]) => n.toLowerCase() === s)) {
    rows.push(
      `<div class="sg${active === matches.length ? " active" : ""}" data-url="${looksLikeUrl(s) ? toUrl(s) : googleSearch(q)}"><span class="fav">${looksLikeUrl(s) ? "🌐" : "🔎"}</span><span class="name">${looksLikeUrl(s) ? "Go to " + q : "Search Google for “" + q + "”"}</span></div>`,
    );
  }
