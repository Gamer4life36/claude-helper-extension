...
async function put(k: any, v: any): Promise<any> {
  const db = await idb();
...
async function get(k: any): Promise<any> {
  const db = await idb();
...
function renderList() {
  ...
  $("list")
    .querySelectorAll(".fi")
    .forEach((el: any) => (el.onclick = () => openFile(+el.dataset.i)));
}
...
async function openFile(i) {
  ...
  $("list")
    .querySelectorAll(".fi")
    .forEach((el: any) => el.classList.toggle("active", +el.dataset.i === i));
  try {
