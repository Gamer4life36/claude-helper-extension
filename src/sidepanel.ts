...
function fuzzyRef(items, q, keys) {
  try {
    const hit = new Fuse(items, { keys, threshold: 0.4 }).search(q)[0];
    return hit ? { ref: (hit.item as any).ref } : null;
  } catch {
    return null;
  }
}
function findEl(q) {
  ...
  const best = lastEls
    .map((x) => ({ x, sc: scoreEl(x, s, ["label", "text", "name", "placeholder", "href"]) }))
    .filter((o) => o.sc > 0)
...
function findField(k) {
  ...
  const best = inputs
    .map((x) => ({ x, sc: scoreEl(x, s, ["label", "name", "placeholder"]) }))
    .filter((o) => o.sc > 0)
    .sort((a, b) => b.sc - a.sc)[0];
  if (best) return { ref: best.x.ref };
...
async function nanoAsk(prompt: any, onProg?: any) {
...
async function nanoPickSkill(request, skills) {
  const list = Object.entries(skills)
    .map(([n, s]: any) => `- ${n}: ${s.description}`)
    .join("\n");
  const ans = (await nanoAsk(`Route the request to ONE browser skill or none.\nSkills:\n${list}\n\nRequest: "${request}"\n\nReply with ONLY the matching skill name, or "none".`))
    .toLowerCase()
    .replace(/[^a-z0-9-]/g, "");
...
async function interpret(text: any, depth: any = 0, opts: any = {}): Promise<any> {
  ...
  if ((m = t.match(/^(?:run|play|macro)\s+([\w-]+)$/i))) {
    ...
    for (const step of steps) {
      const r = await interpret(step, depth + 1);
      addMsg("ai", r.text, r.els ? "els" : r.blocked ? "blocked" : "");
      if (r.blocked) return { text: `Macro “${m[1]}” stopped — a step was blocked.`, blocked: true };
    }
  ...
  if (/^(reader|reader view|read this|clean view|declutter|simplify)( (this|the) page)?$/i.test(l)) {
    ...
    await exec("open_tab", { url: readerPage(r.title, r.text, r.html || "") });
  ...
  if (/^copy (the )?(page )?(as )?(markdown|md)$/i.test(l)) {
    const r = await exec("extract", {});
    if (!r.ok) return { text: "🚫 " + r.error };
    const md = r.html && r.html.trim() ? new TurndownService().turndown(r.html) : r.text || "";
    const c = await exec("copy", { text: md.slice(0, 100000) });
    return { text: c.ok ? "📋 Copied the page as Markdown." : "🚫 " + c.error };
  }
  ...
  if (l === "read" || /^(read|scan)( the)?( page)?$/.test(l) || /what.?s on/.test(l)) {
    ...
    const list = lastEls
      .slice(0, 60)
      .map((x) => `#${x.ref} <${x.tag}${x.type ? " " + x.type : ""}> ${x.label || x.text || x.placeholder || x.name || x.href}`.trim())
      .join("\n");
  ...
  if ((m = l.match(/^click(?:\s+on)?\s+(.+)/))) {
    ...
    const q = m[1]
      .replace(/^the\s+/, "")
      .replace(/\s+(button|link|tab|icon|option)$/, "")
      .trim();
    const tgt = findEl(q);
...
function updateMode() {
  ...
  if (mode === "free") modeEl.innerHTML = '<span class="dot b"></span>Free mode — pattern commands, no AI';
    ...
    modeEl.innerHTML = ready
      ? '<span class="dot g"></span>On-device AI ready — Gemini Nano, local & free'
      : nanoState === "downloadable" || nanoState === "after-download"
        ? '<span class="dot" style="background:#e0a800"></span>On-device AI — model downloads on first use'
        : nanoState === "downloading"
          ? '<span class="dot" style="background:#e0a800"></span>On-device AI — downloading model…'
...
document.getElementById("mFree").onclick = () => {
  setMode("free");
  addMsg("sys", "🔵 Free mode — pattern commands (no AI).");
};
document.getElementById("mNano").onclick = async () => {
  setMode("nano");
  await refreshNano();
  addMsg("sys", "🧠 On-device AI — local Gemini Nano. Ask in plain English (commands still work). First use downloads the model once.");
};
document.getElementById("mApi").onclick = () => {
  setMode("api");
  addMsg("sys", "🟢 Claude API — needs the bridge running with your API key.");
};
...
async function runNano(prompt) {
  ...
  try {
    const ans = await nanoAsk(prompt, (frac) => {
      div.textContent = `🧠 downloading model… ${Math.round(frac * 100)}% (one-time)`;
      logEl.scrollTop = logEl.scrollHeight;
    });
    div.textContent = ans;
    logEl.scrollTop = logEl.scrollHeight;
